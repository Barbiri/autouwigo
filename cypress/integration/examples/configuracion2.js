...
{   
   ...
   {
      cy.get('a > span').contains('Configuración').click()
      cy.get('a[href*="atributo-contable"]').contains('Atributos').click()
      ...
      var nombres = ['Asesorias', 'Capacitaciones', 'Desarrollo Proyectos', 'Servicio Armado','Notebook','Accesorios','Impresoras','Repuestos']
       ...
       cy.get('#id_descripcion').type(nombres[index - 1])
  ...
  it('ATRIBUTOS PROVEEDORES', function() 
   {
    ...
    it('ATRIBUTOS TIPO DE GASTOS', function() //Tipo Gasto
    {
       cy.get('a > span').contains('Configuración').click()
       cy.get('a[href*="atributo-contable"]').contains('Atributos').click()
       var genArr = Array.from({length:37},(v,k)=>k+1)
       var codigos = ['7-700', '7-701', '7-703','7-704','7-705','7-706','7-707','7-708','7-709','7-710','7-711','7-712','7-713','7-714','7-715',
       '7-716','7-717','7-718','7-719','7-720','7-721','7-722','7-723','7-724','7-725','7-726','7-750','7-751','7-752','7-753','7-754','7-755',
       '7-756','7-757','7-758','7-759','7-760','7-761']
       var tgastos = ['Gastos Notariales', 'Gastos Energía Eléctrica', 'Gastos Telefonía', 'Gastos Internet','Gasto Arriendo Oficina',
       'Gasto Asesorías', 'Marketing y Publicidad','Gasto Seguros', 'Licencias Software','Licencias Software','Proporcionalidad IVA CF',
       'Asesorias Evaluacion Hardware','Asesorias Evaluacion Software','Servicios Armado Equipos','Asesorias Implementacion Hardware',
       'Asesorias Implementacion Software','Asesoria Seguridad Informatica','Asesoria e Implementacion Seguridad Informatica','Capacitaciones Office 360 - Word',
       'Capacitaciones Office 360 - Excel','Capacitaciones Office 360 - PowerPoint','Capacitaciones Office 360 - Access','Desarrollo de Sistemas',
        'Evaluacion Desarrollo Sistemas','Desarrollo IA','Ingenieria proyectos','Equipos Computacionales','Sueldos','Ticket Restaurant',
        'Bonos','Provisión Indemnizaciones','Movilización','Atrasos','Gratificación','Sobre Tiempo 50%','Sobre Tiempo 100%','Aguinaldos',
        'Comisión Vendedores','Descuentos Otorgados']
        cy.wrap(genArr).each((index) => {
        cy.get('.col-md-12 > .btn-white').click()
        cy.get('#id_tipo_atributo').select('Item',{force: true})
        cy.get('#id_codigo').type(codigos[index - 1])
        cy.get('#id_descripcion').type(tgastos[index - 1])
        cy.get('button[type=submit]').contains('Guardar').click()
        cy.wait(3000)
    })
   })
